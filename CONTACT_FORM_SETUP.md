...
2. **Grant Cloud Run Service Account Access**:
   ...
   SERVICE_ACCOUNT=$(gcloud run services describe carusoma-website \
     --region=us-central1 \
...
- **Production**: Uses Cloud Storage (bucket: `carusoma-contact-submissions`)
...
3. **Check Cloud Run logs**:
   ```bash
   gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=carusoma-website" --limit 50
   ```
