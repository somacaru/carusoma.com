...
# Create the bucket
gcloud storage buckets create gs://carusoma-contact-submissions \
...
gcloud storage buckets add-iam-policy-binding gs://carusoma-contact-submissions \
...
steps:
  ...
  - name: 'gcr.io/google.com/cloudsdktool/cloud-sdk'
    ...
    args:
      ...
      # ... other args ...
      - '--set-env-vars'
      - 'GCS_BUCKET_NAME=carusoma-contact-submissions'
```
...
gcloud run services update carusoma-website \
  --region=us-central1 \
  --set-env-vars="GCS_BUCKET_NAME=carusoma-contact-submissions"
```
...
2. **Check Cloud Storage**:
   ```bash
   gcloud storage ls gs://carusoma-contact-submissions/
   ```
3. **View the file**:
   ```bash
   gcloud storage cat gs://carusoma-contact-submissions/contact-submissions.json
   ```

### Check Logs

```bash
gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=carusoma-website" \
  --limit 50 \
  --format json
```
...
1. **Check bucket exists**:
   ```bash
   gcloud storage buckets list --filter="name:carusoma-contact-submissions"
   ```

2. **Check permissions**:
   ```bash
   gcloud storage buckets get-iam-policy gs://carusoma-contact-submissions
   ```

3. **Check environment variable**:
   ```bash
   gcloud run services describe carusoma-website \
     --region=us-central1 \
